...
public class AccountDepositServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (type != null && "euro".equals(type[0])) {
			acccountService.depositEuros(Long.parseLong(accountNumber), Double.parseDouble(amount));
		} else {
			acccountService.deposit(Long.parseLong(accountNumber), Double.parseDouble(amount));
		}
